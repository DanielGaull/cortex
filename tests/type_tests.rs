...
use cortex_lang::{
    ...
    parsing::{
        ast::{
            expression::{OptionalIdentifier, PExpression, Parameter, PathIdent},
            top_level::{BasicBody, Body, Contract, MemberFunction, PFunction, Struct},
        },
        codegen::r#trait::SimpleCodeGen,
        parser::CortexParser,
    },
    preprocessing::{module::Module, preprocessor::preprocessor::CortexPreprocessor},
    r#type::r#type::{FollowsClause, FollowsEntry, PType, TypeArg, TypeParam, TypeParamType},
};
...
fn subtype_tests() -> Result<(), Box<dyn Error>> {
    ...
    module.add_contract(Contract::new(
        "Container",
    ...
    module.add_struct(Struct::new(
        ...
        vec![],
        vec![],
        Some(FollowsClause::new(vec![FollowsEntry::new(
            PathIdent::new(vec!["Iterable"]),
            vec![],
        )])),
    ))?;
    module.add_struct(Struct::new(
    ...
    module.add_struct(Struct::new(
        ...
        vec!["T"],
        Some(FollowsClause::new(vec![FollowsEntry::new(
            PathIdent::new(vec!["Container"]),
            vec![TypeArg::Ty(PType::generic("T"))],
        )])),
    ))?;
    ...
    assert_not_subtype(
        "Box<TestType>",
        "Box<follows Iterable>",
        &preprocessor,
        Some("Box<follows Iterable>"),
    )?;
...
fn run_reference_type_tests() -> Result<(), Box<dyn Error>> {
    ...
    module.add_struct(Struct::new(
        ...
        vec![(
            "time",
            PType::reference(
                PType::basic(PathIdent::simple(String::from("Time")), vec![]),
                true,
            ),
        )],
        vec![MemberFunction::new(
            OptionalIdentifier::Ident(String::from("get")),
            vec![],
            PType::reference(PType::simple("Time"), true),
            Body::Basic(BasicBody::new(
                vec![],
                Some(CortexParser::parse_expression("this.time")?),
            )),
    ...
    interpreter.execute_statement(CortexParser::parse_statement(
        "let box = heap Time::Box{ time: heap Time::Time{m:4,s:5} };",
    )?)?;
...
fn run_generic_type_tests() -> Result<(), Box<dyn Error>> {
    ...
    module.add_struct(Struct::new(
        ...
        vec![MemberFunction::new(
            ...
            Body::Basic(BasicBody::new(
                vec![],
                Some(CortexParser::parse_expression("this.item")?),
            )),
...
fn run_function_pointer_tests() -> Result<(), Box<dyn Error>> {
    ...
    module.add_function(PFunction::new(
        ...
        vec![Parameter::named(
            "t",
            PType::OptionalType(Box::new(PType::i32())),
        )],
...
fn assert_subtype(
    ...
    if let Some(combined_str) = combined_str {
        ...
        let combined_type = preprocessor
            .combine_types(t1, t2)?
            .expect(&format!("'{}' cannot be combined with '{}'", first, second));
        assert_eq!(
            combined,
            combined_type,
            "'{}' combined with '{}' => '{}', which isn't equal to '{}'",
            first,
            second,
            combined_type.codegen(0),
            combined_str
        );
    } else {
        assert!(
            preprocessor.combine_types(t1, t2)?.is_none(),
