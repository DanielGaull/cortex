...
use cortex_lang::{
    interpreting::{
        env::EnvError, error::InterpreterError, interpreter::CortexInterpreter, value::CortexValue,
    },
    parsing::{
        ast::{
            expression::{OptionalIdentifier, PExpression, Parameter},
            top_level::{BasicBody, Body, PFunction, Struct},
        },
        parser::{CortexParser, ParseError},
    },
    preprocessing::{
...
fn test_variable_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "dneVar = 7;",
        EnvError::VariableDoesNotExist(String::from("dneVar")),
        &mut interpreter,
    )?;
    interpreter.execute_statement(CortexParser::parse_statement("const x = 5;")?)?;
    assert_err(
        "let x = 7;",
        EnvError::VariableAlreadyExists(String::from("x")),
        &mut interpreter,
    )?;
    assert_err(
        "x = 7;",
        PreprocessingError::CannotModifyConst(String::from("x")),
        &mut interpreter,
    )?;
...
fn test_operator_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "5 - \"foo\";",
        PreprocessingError::InvalidOperator(
            "number",
            "number",
            "-",
            String::from("i32"),
    ...
    assert_err(
        "5.2 * \"foo\";",
        InterpreterError::ExpectedInteger(5.2f64),
        &mut interpreter,
    )?;
    Ok(())
}
...
fn test_function_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "simple::hi();",
        PreprocessingError::FunctionDoesNotExist(String::from("simple::hi")),
        &mut interpreter,
    )?;
    assert_err(
        "simple::add(1);",
        PreprocessingError::MismatchedArgumentCount(String::from("simple::add"), 2, 1),
        &mut interpreter,
    )?;
    assert_err(
        "simple::add(1, 2, 3);",
        PreprocessingError::MismatchedArgumentCount(String::from("simple::add"), 2, 3),
        &mut interpreter,
    )?;
    assert_err(
        "simple::add(1, true);",
        PreprocessingError::MismatchedType(
            ...
            String::from("bool"),
            String::from("b"),
            String::from("simple::add(1, true)"),
        ),
        &mut interpreter,
    )?;
    assert_err(
        "simple::generic<i32>(true);",
        PreprocessingError::MismatchedType(
            String::from("i32"),
            String::from("bool"),
            String::from("t"),
            String::from("simple::generic<i32>(true)"),
        ),
        &mut interpreter,
    )?;
    Ok(())
...
fn test_contract_errors() -> Result<(), Box<dyn Error>> {
    ...
    interpreter.run_top_level(CortexParser::parse_top_level(
        "contract NetworkRequester {
        fn next(&this): string;
    }",
    )?)?;

    interpreter.run_top_level(CortexParser::parse_top_level(
        "struct NumList follows Iterator<i32> {
        i: i32,
        values: &list<i32>,
    ...
    assert_err_toplevel(
        ...
        PreprocessingError::ContractFunctionsMissing(String::from("next")),
        &mut interpreter,
    )?;
    assert_err_toplevel(
        "extend i32 follows Iterator<i32> {}",
        PreprocessingError::ContractFunctionsMissing(String::from("next")),
        &mut interpreter,
    )?;
    assert_err_toplevel(
        "struct NumList2 follows NumIterator {}",
        PreprocessingError::ContractDoesNotExist(String::from("NumIterator")),
        &mut interpreter,
    )?;
    assert_err_toplevel(
        "struct NumList3 follows Iterator<i32> + NetworkRequester {
        fn next(&mut this): i32 {
            5
        }
    }",
        PreprocessingError::AmbiguousFunctionFromMultipleContracts(String::from("next")),
        &mut interpreter,
    )?;
    assert_err_toplevel(
        "struct NumList4 follows NetworkRequester + NetworkRequester {
        fn next(&this): i32 {
            5
        }
    }",
        PreprocessingError::DuplicateInFollowsClause(String::from("NetworkRequester")),
        &mut interpreter,
    )?;
    ...
    interpreter.execute_statement(CortexParser::parse_statement(
        "let x: follows Iterator<i32> = heap NumList { i: 0, values: [1, 2] };",
    )?)?;
    assert_err(
        "x.dne();",
        PreprocessingError::FunctionDoesNotExist(String::from("dne")),
        &mut interpreter,
    )?;
    assert_err(
        "x.i;",
...
fn test_composite_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "simple::Time { z: 5 };",
        PreprocessingError::FieldDoesNotExist(String::from("z"), String::from("simple::Time")),
        &mut interpreter,
    )?;
    interpreter.execute_statement(CortexParser::parse_statement(
        "let myTime = simple::Time { m: 5, s: 2 };",
    )?)?;
    assert_err(
        "myTime.z;",
        PreprocessingError::FieldDoesNotExist(String::from("z"), String::from("simple::Time")),
    ...
    assert_err(
        ...
        PreprocessingError::MismatchedType(
            String::from("i32"),
            String::from("bool"),
            String::from("myTime.m"),
            String::from("myTime.m = true;"),
        ),
        &mut interpreter,
    )?;
    assert_err(
    ...
    assert_err(
        "dneStruct { foo: 5 };",
        PreprocessingError::TypeDoesNotExist(String::from("dneStruct")),
        &mut interpreter,
    )?;
    assert_err(
        "simple::Time { m: 2 };",
        PreprocessingError::NotAllFieldsAssigned(String::from("simple::Time"), String::from("s")),
        &mut interpreter,
    )?;
    assert_err(
        "simple::Time { m: 2, m: 3 };",
        PreprocessingError::MultipleFieldAssignment(String::from("m")),
        &mut interpreter,
    )?;
    interpreter.execute_statement(CortexParser::parse_statement(
        "let box: &simple::IntBox = heap simple::IntBox { v: 100 };",
    )?)?;
    assert_err(
        "box.v = 7;",
        PreprocessingError::CannotModifyFieldOnImmutableReference(String::from("simple::IntBox")),
        &mut interpreter,
    )?;
    assert_err(
        "5.hello();",
        PreprocessingError::FunctionDoesNotExist(String::from("hello (on type i32)")),
        &mut interpreter,
    )?;
    Ok(())
}
...
fn nullable_member_access_tests() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "x.t0;",
        PreprocessingError::CannotAccessMemberOfOptional(String::from("x")),
        &mut interpreter,
    )?;
    interpreter.execute_statement(CortexParser::parse_statement(
        "let myTime: simple::Time? = simple::Time { m: 5, s: 2 };",
    )?)?;
    assert_err(
        "myTime.m;",
        PreprocessingError::CannotAccessMemberOfOptional(String::from("myTime")),
        &mut interpreter,
    )?;
    Ok(())
}
...
fn test_none_related_errors() -> Result<(), Box<dyn Error>> {
    let mut interpreter = setup_interpreter()?;
    assert_err(
        "let notOptional: i32 = none;",
        PreprocessingError::MismatchedType(
            String::from("i32"),
            String::from("none"),
            String::from("notOptional"),
            String::from("let notOptional: i32 = none;"),
        ),
        &mut interpreter,
    )?;
    assert_err(
        "none!;",
        InterpreterError::BangCalledOnNoneValue,
        &mut interpreter,
    )?;
    Ok(())
}
...
fn test_conditional_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "if true { 5 } else { \"hi\" };",
        PreprocessingError::IfArmsDoNotMatch(String::from("i32"), String::from("string")),
        &mut interpreter,
    )?;
    assert_err(
        "if true { 5 } elif true { 1 };",
        PreprocessingError::IfRequiresElseBlock,
        &mut interpreter,
    )?;
    assert_err(
        "while true { 5 }",
...
fn test_tuple_var_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        ...
        PreprocessingError::MismatchedType(
            ...
            String::from("(i32, i32)"),
            String::from("$temp0"),
            String::from("const $temp0: i32 = (1, 2);"),
        ),
        &mut interpreter,
    )?;
    assert_err(
        "let (x, y): (bool, bool) = (1, 2);",
        PreprocessingError::MismatchedType(
            String::from("(bool, bool)"),
            String::from("(i32, i32)"),
            String::from("$temp1"),
            String::from("const $temp1: (bool, bool) = (1, 2);"),
        ),
        &mut interpreter,
    )?;
    Ok(())
...
fn assert_err<T: Error + PartialEq + 'static>(
    statement: &str,
    flavor: T,
...
fn test_other_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "throw 5;",
        InterpreterError::ProgramThrow(CortexValue::I32(5)),
        &mut interpreter,
    )?;
    ...
    assert_err_toplevel(
        "fn f(): void {}",
        ModuleError::FunctionAlreadyExists(String::from("f")),
        &mut interpreter,
    )?;
    assert_err_toplevel(
        "module myMod{ module m{} module m{} }",
        ModuleError::ModuleAlreadyExists(String::from("m")),
        &mut interpreter,
    )?;

    assert_err_toplevel(
        "struct A{a:A}",
        PreprocessingError::StructContainsCircularFields(String::from("A")),
        &mut interpreter,
    )?;
    ...
    assert_err_toplevel(
        "struct s{}",
        ModuleError::StructAlreadyExists(String::from("s")),
        &mut interpreter,
    )?;

    assert_err_equal(
        CortexParser::parse_top_level("struct A{a:i32, a:i32}")
            .map_err(|e| Box::new(e) as Box<dyn Error>),
        ParseError::CompositeContainsDuplicateFields(String::from("A"), String::from("a")),
    )?;

    interpreter.run_top_level(CortexParser::parse_top_level("contract c{}")?)?;
    assert_err_toplevel(
        "contract c{}",
        ModuleError::ContractAlreadyExists(String::from("c")),
        &mut interpreter,
    )?;

    assert_err(
        "let myList = [];",
        PreprocessingError::CannotDetermineType(String::from("[]")),
        &mut interpreter,
    )?;
...
fn test_tuple_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err(
        "myTuple.x;",
        PreprocessingError::TupleMemberSyntaxInvalid(String::from("x")),
        &mut interpreter,
    )?;
    assert_err(
        "myTuple.t12;",
        PreprocessingError::TupleIndexValueInvalid(2, 12),
        &mut interpreter,
    )?;
    Ok(())
}
...
fn test_type_argument_errors() -> Result<(), Box<dyn Error>> {
    ...
    assert_err_toplevel(
        "fn b<T, T>(){}",
        ModuleError::DuplicateTypeArgumentName(String::from("T")),
        &mut interpreter,
    )?;
    assert_err_toplevel(
        "struct www<T, T>{}",
...
fn assert_err_toplevel<T: Error + PartialEq + 'static>(
    statement: &str,
    flavor: T,
    interpreter: &mut CortexInterpreter,
) -> Result<(), Box<dyn Error>> {
...
fn setup_interpreter() -> Result<CortexInterpreter, Box<dyn Error>> {
    ...
    let generic_func = PFunction::new(
        ...
        vec![Parameter::named("t", PType::generic("T"))],
    ...
    let test_struct2 = Struct::new("IntBox", vec![("v", PType::i32())], vec![], vec![], None);
