...
use crate::{
    parsing::{
        ast::{
            expression::{
                BinaryOperator, IdentExpression, OptionalIdentifier, PConditionBody, PExpression,
                PathIdent, UnaryOperator,
            },
            statement::{AssignmentName, DeclarationName, PStatement},
            top_level::{BasicBody, Body, PFunction},
        },
    ...
    preprocessing::ast::{
        ...
        top_level::RContract,
        r#type::{RType, RTypeArg},
    },
    r#type::{
        r#type::{PType, TypeArg, TypeParam},
        type_checking_env::TypeCheckingEnvironment,
...
use super::super::{
    ast::{
        ...
        function::{FunctionDict, RBody, RDefinedBody, RFunction},
        function_address::FunctionAddress,
        statement::{RConditionBody, RStatement},
    },
    error::PreprocessingError,
    module::{Module, TypeDefinition},
    program::Program,
};
...
pub struct CortexPreprocessor {
    ...
    pub(super) modules: Vec<(PathIdent, Module)>,
}
...
impl CortexPreprocessor {
    ...
    pub(super) fn assign_to(
        ...
        base_type: RType,
        typ: RType,
    ) -> Result<(RExpression, Vec<RStatement>), CortexError> {
        match (typ, base_type) {
            ...
            (RType::TupleType(t), base_type) => {
                ...
                self.current_env
                    .as_mut()
                    .unwrap()
                    .add(temp_tup.clone(), base_type, true)?;
                ...
                for (i, entry) in t.into_iter().enumerate() {
                    ...
                    let (member_access_exp, member_access_typ, st) =
                        self.check_exp(member_access_exp, None)?;
                    statements.extend(st);
                    let (value, st) =
                        self.assign_to(member_access_exp, member_access_typ, entry)?;
                    statements.extend(st);
        ...
        match function.body {
            Body::Basic(body) => {
                ...
                if !self.is_subtype(&body_type, &return_type)? {
                    return Err(Box::new(PreprocessingError::ReturnTypeMismatch(
    ...
    fn check_statement(&mut self, statement: PStatement) -> Result<Vec<RStatement>, CortexError> {
        ...
        match statement {
            ...
            PStatement::VariableDeclaration {
                name,
                is_const,
                typ,
                initial_value,
            ...
            PStatement::WhileLoop(condition_body) => {
                let (cond, cond_type, mut cond_statements) =
                    self.check_exp(condition_body.condition, Some(RType::boolean()))?;
                ...
                self.loop_depth += 1;
                let (body, body_type) =
                    self.check_body_and_handle_env(condition_body.body, Some(RType::void()))?;
                if !self.is_subtype(&body_type, &RType::void())? {
                    return Err(Box::new(PreprocessingError::LoopCannotHaveReturnValue));
                }
                self.loop_depth -= 1;
    ...
    fn check_declaration(
        ...
        st_str: &String,
    ) -> Result<Vec<RStatement>, CortexError> {
        match name {
            OptionalIdentifier::Ident(ident) => {
                let (assigned_exp, assigned_type, mut statements) =
                    self.check_exp(initial_value, typ.clone())?;
                ...
                self.current_env.as_mut().unwrap().add(
                    ident.clone(),
                    type_of_var.clone(),
                    is_const,
                )?;

                let (initial_value, assign_st) =
                    self.assign_to(assigned_exp, assigned_type, type_of_var)?;
    ...
    fn check_declaration_recursive(
        &mut self,
        ...
        match name {
            ...
            DeclarationName::Tuple(names) => {
                ...
                for (i, name) in names.into_iter().enumerate() {
                    let new_value =
                        PExpression::MemberAccess(Box::new(temp_expr.clone()), format!("t{}", i));
                    result.extend(
                        self.check_declaration_recursive(name, None, is_const, new_value, &st_str)?,
                    );
                }
                Ok(result)
            }
    ...
    fn check_assignment(
        ...
        if name.is_simple() {
            ...
            let (aassigned_exp, aassigned_type, astatements) =
                self.check_exp(value, Some(type_of_var.clone()))?;
            ...
            if self.current_env.as_ref().unwrap().is_const(var_name)? {
                return Err(Box::new(PreprocessingError::CannotModifyConst(
                    var_name.clone(),
                )));
            }
        } else {
        ...
        statements.extend(assign_st);
        statements.push(RStatement::Assignment {
            name: name.into(),
            value: assigned_exp,
        });
        Ok(statements)
    }
    fn check_assignment_recursive(
        &mut self,
        name: AssignmentName,
        value: PExpression,
        st_str: &String,
        ...
        match name {
            AssignmentName::Single(name) => Ok(self.check_assignment(name, value, &st_str)?),
            AssignmentName::Ignore => Ok(self.check_statement(PStatement::Expression(value))?),
        ...
        match exp {
            ...
            PExpression::PathIdent(path_ident) => {
                if self.has_variable(&path_ident) {
                    Ok((
                        RExpression::Identifier(path_ident.get_back()?.clone()),
                        self.get_variable_type(&path_ident)?,
                        vec![],
                    ))
                } else {
                    ...
                    if self.function_dict.exists(&function_addr) {
                        ...
                        Err(Box::new(PreprocessingError::ValueNotFound(
                            path_ident.codegen(0),
                        )))
                    }
                }
            }
            ...
            PExpression::UnaryOperation { op, exp } => match op {
                UnaryOperator::Negate => {
                    let (exp, typ, statements) = self.check_exp(*exp, expected_type)?;
                    if typ == RType::i32() {
                        Ok((
                            RExpression::UnaryOperation {
                        ...
                        Err(Box::new(PreprocessingError::InvalidOperatorUnary(
                            "i32",
                            "-",
                            typ.codegen(0),
                        )))
            ...
            PExpression::CollectionLiteral(items) => {
                ...
                let mut statements = Vec::new();
                for item in items {
                    let (item_exp, item_type, exp_st) =
                        self.check_exp(item, expected_internal.clone())?;
                    ...
                    if let None = contained_type {
                        ...
                        contained_type = Some(self.combine_types(typ, item_type)?.ok_or(
                            PreprocessingError::CannotDetermineCollectionLiteralType(
                                typ_str,
                                item_type_str,
                            ),
                        )?);
                ...
                if let Some(expected) = expected_internal {
                    if let Some(contained) = contained_type {
                        if self.is_subtype(&contained, &expected)? {
                            ...
                            Ok((
                                RExpression::CollectionLiteral(new_items),
                                true_type,
                                statements,
                            ))
                        } else {
            ...
            PExpression::Bang(inner) => {
                let (exp, typ, statements) =
                    self.check_exp(*inner, expected_type.map(|t| t.to_optional()))?;
                Ok((
                    RExpression::Bang(Box::new(exp)),
                    typ.to_non_optional(),
                    statements,
                ))
            }
            PExpression::MemberAccess(inner, member) => {
                ...
                match &atom_type {
                    RType::BasicType(..) | RType::RefType(..) => {
                        if atom_type.is_non_composite() {
                            return Err(Box::new(
                                PreprocessingError::CannotAccessMemberOfNonComposite,
                            ));
                        }
                        if atom_type.optional() {
                            return Err(Box::new(
                                PreprocessingError::CannotAccessMemberOfOptional(inner_as_string),
                            ));
                        }
                        let (ex, ty, st) =
                            self.check_composite_member_access(atom_exp, atom_type, member)?;
                        statements.extend(st);
                        Ok((ex, ty, statements))
                    }
                    RType::TupleType(t) => {
                    ...
                    RType::FollowsType(..) => Err(Box::new(
                        PreprocessingError::CannotAccessMemberOfFollowsType,
                    )),
                    RType::OptionalType(..) => Err(Box::new(
                        PreprocessingError::CannotAccessMemberOfOptional(inner_as_string),
                    )),
                    RType::NoneType | RType::FunctionType(..) | RType::GenericType(..) => Err(
                        Box::new(PreprocessingError::CannotAccessMemberOfNonComposite),
                    ),
                    // Theoretically, we should only ever try to access a member on a This type
                    // if we're in a contract. It will resolve to a proper type in a struct
                    RType::ThisType => Err(Box::new(
                        PreprocessingError::CannotAccessMemberOfFollowsType,
                    )),
                }
            }
            PExpression::MemberCall {
                ...
                if let RType::FollowsType(f) = atom_type {
                    let (ex, ty, st) =
                        self.check_fat_member_call(f, callee, member, args, type_args, st_str)?;
                    statements.extend(st);
                    Ok((ex, ty, statements))
                } else {
            ...
            PExpression::Range { start, end, step } => {
                fn otov(o: Option<isize>) -> RExpression {
                    match o {
                        Some(v) => RExpression::ISZ(v),
            ...
            PExpression::HeapAlloc(exp) => {
                ...
                Ok((
                    RExpression::HeapAlloc(Box::new(sub)),
                    RType::reference(typ, true),
                    st,
                ))
            }
            ...
            PExpression::DeAnon(typ, inner) => {
                ...
                if inner_type != RType::anonbox() {
                    return Err(Box::new(PreprocessingError::CannotDeanonymize(
                        inner_type.codegen(0),
                    )));
                }
    ...
    fn check_composite_member_access(
        &mut self,
        atom_exp: RExpression,
        atom_type: RType,
        member: String,
    ) -> CheckResult<RExpression> {
        let is_mutable;
        ...
        if !typedef.fields.contains_key(&member) {
            Err(Box::new(PreprocessingError::FieldDoesNotExist(
                member.clone(),
                atom_type.codegen(0),
            )))
        } else {
            ...
            Ok((
                RExpression::MemberAccess(Box::new(atom_exp), member),
                member_type,
                vec![],
            ))
        }
    }
    fn check_tuple_member_access(
        &mut self,
        atom_exp: RExpression,
        atom_type: &Vec<RType>,
        member: String,
        ...
        if index > atom_type.len() {
            return Err(Box::new(PreprocessingError::TupleIndexValueInvalid(
                atom_type.len(),
                index,
            )));
        }
        ...
        Ok((
            RExpression::MemberAccess(Box::new(atom_exp), format!("t{}", index)),
            member_type,
            vec![],
        ))
    }
    ...
    fn check_construction(
        ...
        let (type_params, true_path) = self
            .get_struct_stub(&name)
            .ok_or(PreprocessingError::TypeDoesNotExist(name.codegen(0)))?;
        let type_args =
            self.validate_type_args(&type_params, type_args, true_path.codegen(0), "Type")?;
        ...
        for (fname, fvalue) in assignments {
            let opt_typ = fields.get(&fname).map(|t| t.clone());
            if let Some(typ) = opt_typ {
                ...
                if let Some(index) = index_opt {
                    ...
                    return Err(Box::new(PreprocessingError::MultipleFieldAssignment(
                        fname.clone(),
                    )));
                }
            } else {
                return Err(Box::new(PreprocessingError::FieldDoesNotExist(
                    fname.clone(),
                    name.codegen(0),
                )));
            }
        ...
        if fields_to_assign.is_empty() {
            ...
            Err(Box::new(PreprocessingError::NotAllFieldsAssigned(
                name.codegen(0),
                fields_to_assign.join(","),
            )))
        }
    }
    fn check_if_statement(
        &mut self,
        first: PConditionBody,
        conds: Vec<PConditionBody>,
        last: Option<BasicBody>,
        st_str: &String,
    ) -> CheckResult<RExpression> {
        let (cond_exp, cond_typ, mut pre_statements) =
            self.check_exp(first.condition, Some(RType::boolean()))?;
        ...
        for c in conds {
            ...
            if let Some(t) = next {
                ...
                return Err(Box::new(PreprocessingError::IfArmsDoNotMatch(
                    the_type_str,
                    typ_str,
                )));
            }
    ...
    fn check_body_and_handle_env(
        &mut self,
        body: BasicBody,
        expected_return_value: Option<RType>,
    ) -> Result<(RDefinedBody, RType), CortexError> {
        let parent_env = self
            .current_env
            .take()
            .ok_or(PreprocessingError::NoParentEnv)?;
        ...
        match op {
            BinaryOperator::Add => {
                if numbers.contains(&first) && numbers.contains(&second) && first == second {
                    ...
                    Err(Box::new(PreprocessingError::InvalidOperator(
                        "number, string",
                        "number, string",
                        "+",
                        first.codegen(0),
            ...
            BinaryOperator::IsEqual => Ok(boolean),
            BinaryOperator::IsNotEqual => Ok(boolean),
